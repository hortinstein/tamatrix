...
static char cacheValid=0;
static char cachedHostname[128]="";
static ip_addr_t cachedAddr;
...
static void ICACHE_FLASH_ATTR dns_callback(const char * hostname, ip_addr_t * addr, void * arg)
{
	...
	if (addr == NULL) {
		...
		cacheValid=1;
		memcpy(&cachedAddr, addr, sizeof(ip_addr_t));
...
void ICACHE_FLASH_ATTR http_raw_request(const char * hostname, int port, bool secure, const char * path, const char * post_data, const char * headers, http_callback user_callback)
{
	...
	if (strcmp(hostname, cachedHostname)!=0 && cacheValid) {
		cacheValid=0;
		...
		memcpy(&addr, &cachedAddr, sizeof(addr));
		error=ESPCONN_OK;
	}
